...
describe('canAssignToUser', () => {
  ...
  it('denies a reports-only actor targeting themselves', () => {
    const actor = holder(CAPABILITY.LMS_LEADS_ASSIGN, CAPABILITY.LMS_LEADS_ASSIGN_REPORTS);
    expect(canAssignToUser(actor, LMS_RANKS.SSE, LMS_RANKS.SSE, ACTOR_ID, ACTOR_ID)).toBe(false);
  });
  ...
  it('denies an actor with none of the assign scopes', () => {
